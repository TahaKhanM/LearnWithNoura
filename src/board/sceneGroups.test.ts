...
describe('semantic board sections', () => {
  it('keeps overlapping groups on separate pages and retains global legacy items', () => {
    ...
    expect(sceneForGroup(scene, 'group-two').items.map((item) => item.id)).toEqual(['legacy', 'two-box', 'sketch-two']);
    ...
    expect(sceneForGroup(cleared, 'group-two').items.map((item) => item.id)).toEqual(['legacy', 'sketch-two']);
  });
});
