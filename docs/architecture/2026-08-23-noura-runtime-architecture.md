...
- **Consumption.** The realtime session seeds its lesson state from the stored blueprint (restored on reconnect exactly like before), injects the current stage's objective, checks and storyboard beats into instructions per stage, and resolves the `establish` visual action to the pre-validated anchor scene instead of asking the voice model to invent geometry. `create_lesson_blueprint` no longer exists as a live tool; legacy event parsing remains for replaying old sessions.
