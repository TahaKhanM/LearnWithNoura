...
Released visual checkpoints no longer roll back merely because the generation changes while their draw-on animation is finishing. The animator finishes the already-heard checkpoint within a bounded window and commits it while merging the latest learner-owned layer. Learner stroke/erase BoardOps persist independently and replay with learner ownership. After batched learner activity, the browser sends Realtime a size-bounded JPEG of the full board as an `input_image` conversation item alongside the scene description; the image is provider context only and is not stored by Noura.
