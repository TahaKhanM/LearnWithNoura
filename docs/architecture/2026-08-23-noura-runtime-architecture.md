...
`Educational intent → SemanticScenePlan 1.0.0 → domain template → BoardOps → exact compiler → tutor-layer inspection → one repair → accept/reject → heard visible checkpoint → draw-on animation → replay acknowledgement`.
