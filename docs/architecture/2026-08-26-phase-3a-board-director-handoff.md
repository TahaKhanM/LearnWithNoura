...
| `server/realtime/storyboardRunner.ts` | 452 | The interleaved reveal-narrate engine (size reason above) |
| `server/realtime/visualRequests.ts` | 352 | `request_visual` handling: fast tier, anchor build, Director flow |
...
| `server/realtime/turnFloor.ts` | 122 | `response.create` ownership + in-flight tracking + floor gate |
| `server/lesson/headlessSceneValidator.ts` | 104 | Headless validate + render on one harness page |
| `server/realtime/storyboardRunner.test.ts` | 573 | Runner behavior vs fake sideband/client |
| `server/board/director.test.ts` | 276 | Director scripted-double coverage |
| `server/realtime/storyboardSession.integration.test.ts` | 276 | Full loop: real coordinator ↔ real RealtimeSession |
