...
## Spend incident (not retroactively authorized)
...
- Minimum 11 completed provider requests. Exact call count and USD unknown (compiler usage is not persisted). Do not invent an invoice.
...
`accepted` stays **false**. Typing `ratified` records the incident as process-complete, not authorized after the fact. A decline keeps this blocked record.
...
Do not start M8 or M6. M4/M5 remainder already landed offline. M7 formal acceptance still needs the owner to type `ratified`.
